...
bool initQueue(FrameQueue * WantedFrameQueue, int size) {
    ...
    if (WantedFrameQueue != NULL) {
        delete WantedFrameQueue;
        WantedFrameQueue = NULL;
    }
    if (WantedFrameQueue == NULL) WantedFrameQueue = (FrameQueue*)malloc(sizeof(FrameQueue));
    #endif
    WantedFrameQueue->frames = (Frame**)malloc(size * sizeof(Frame*));
    WantedFrameQueue->frames_array = (Frame*)malloc(size * sizeof(Frame));
    WantedFrameQueue->nextFrameIndex = 0;
    WantedFrameQueue->numberOfFrames = 0;
    WantedFrameQueue->starterIndex = 0;
    if(WantedFrameQueue->frames != NULL && WantedFrameQueue->frames_array != NULL) {
      WantedFrameQueue->maxSize = size;
    } else {
...
bool enqueue(FrameQueue * WantedQueue, Frame * WantedFrame) {
    if(WantedQueue->numberOfFrames < WantedQueue->maxSize) {
        ...
        WantedQueue->frames_array[WantedQueue->nextFrameIndex].frame = WantedFrame->frame;
        WantedQueue->frames_array[WantedQueue->nextFrameIndex].capture_timestamp = WantedFrame->capture_timestamp;
        WantedQueue->frames_array[WantedQueue->nextFrameIndex].capture_timestamp_int = WantedFrame->capture_timestamp_int;
...
void captureFrame(CvCapture * camToCaptureFrom, Frame * FramePlacement) {
    ...
    FramePlacement.capture_timestamp = getTimeMsec(FramePlacement.capture_timestamp_int);
